...
def discovery_network(local_ip_address,port, ip_start_index,ip_end_index,filename):
	...
	cpt = ip_start_index
	cpt_ip_active = 0
	print("Start ip to scan:",prefix_ip + str(cpt),"end ip to scan:",prefix_ip + str(ip_end_index))
	while cpt < ip_end_index:
		ip_to_get =  prefix_ip + str(cpt);
		ip_is_active = socket_connexion(ip_to_get,port)
		print("Check if ip:" ,ip_to_get," is active and port",port,"is open :",ip_is_active)
		cpt = cpt +1
		if ip_is_active:
